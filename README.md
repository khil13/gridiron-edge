...
**Game detail** — live games show the score, quarter and clock in the header, with a **live win probability** that replaces the pregame number rather than leaving it on screen to be mistaken for a current estimate. It scales the remaining game to the time left and narrows the spread of outcomes with the square root of it, so a ten-point lead reads as 27% in the first quarter and 2% in the fourth. It knows nothing about possession, timeouts or field position, so it says so when a one-score game gets inside two minutes. On the Stats tab a live game draws the **field**, with the ball spotted where it actually is and the line to gain marked. A down and distance in text tells you the situation; seeing the ball on your own 4 rather than the opponent's 30 tells you what it means. The possessing team always drives left to right, and end zones fall back to a club's secondary colour when the two primaries are too close to tell apart — 57 of the 496 possible matchups would otherwise render two identical end zones.

Plus a **Stats** tab on live and finished games with the real box score: scoring by quarter, team statistics, game leaders, and for games in progress the current down, distance, possession and last play. It polls while the game is live. Plus market-vs-model breakdown, a full odds comparison across six books with best-price highlighting, line movement since open, win probability charts for finished games, and a Model tab that shows every input that produced the number.
...
Volume markets need a real per-game rate. Before week one nobody has one, so the app falls back to last season's rates — real data rather than a guess — and labels them as last season, because an offseason sits between: players changed teams, roles moved, and a rookie has no line at all because he has no history to project from. They are replaced by this season's numbers as games are played.

Volume markets only appear for players with a real rate. A positional average is adequate for a touchdown share, which is a proportion, and worthless for a yardage line where being twenty yards out is the whole bet. First-quarter *player* props are deliberately not offered: they need drive and snap data this app does not have. You can type in the price your sportsbook is showing and get EV, edge and a stake immediately; no odds key is needed for that, because the price you are actually being offered is the number that decides the bet. Prices you enter are saved per game and survive a reload.
